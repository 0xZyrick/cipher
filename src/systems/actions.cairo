...
pub mod actions {
    ...
    use cipher::models::{
        ...
        PIECES_PER_PLAYER, BOARD_SIZE, P1_MAX_ROW, P2_MIN_ROW, EMPTY_SQUARE,
    ...
    impl ActionsImpl of IActions<ContractState> {
        ...
        fn place_piece(
            ...
            if is_p1 {
                ...
                assert(
                    piece_id >= PIECES_PER_PLAYER && piece_id < PIECES_PER_PLAYER * 2,
                    'Invalid piece id for p2'
                );
            ...
            if is_p1 {
                game.p1_pieces_placed += 1;
            } else {
                game.p2_pieces_placed += 1;
            }
            world.write_model(@game);
        }
        ...
        fn ready(ref self: ContractState, game_id: felt252) {
            ...
            let is_p1 = caller == game.player1;
            if is_p1 {
                assert(game.p1_pieces_placed == PIECES_PER_PLAYER, 'Place all pieces first');
            } else {
                assert(game.p2_pieces_placed == PIECES_PER_PLAYER, 'Place all pieces first');
            }

            if game.p1_pieces_placed == PIECES_PER_PLAYER
                && game.p2_pieces_placed == PIECES_PER_PLAYER {
        ...
        fn move_piece(
            ...
            if !dest_square.is_occupied {
                InternalImpl::do_move(ref world, game_id, piece_id, from_x, from_y, to_x, to_y);
                game.turn_count += 1;
                ...
                world.write_model(@PendingCombat {
                    ...
                    attacker_rank: rank,
                    defender_piece_id: dest_square.piece_id,
        ...
        fn resolve_combat(
            ...
            if rank == RANK_FLAG {
                ...
                ended_game.status = STATUS_FINISHED;
                ended_game.winner = attacker.owner;
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn do_move(
            ...
            piece.y = to_y;
            world.write_model(@piece);
            world.write_model(@Square { game_id, x: from_x, y: from_y, piece_id: EMPTY_SQUARE, is_occupied: false });
        ...
        fn kill_piece(
            ...
            world.write_model(@piece);
            world.write_model(@Square { game_id, x, y, piece_id: EMPTY_SQUARE, is_occupied: false });
        }
        ...
        fn clear_combat(ref world: WorldStorage, game_id: felt252) {
            world.write_model(@PendingCombat {
                game_id,
                is_active: false,
                attacker_piece_id: 0, attacker_rank: 0,
                defender_piece_id: 0, from_x: 0, from_y: 0, to_x: 0, to_y: 0,
            });
        }

        fn validate_scout_move(
            ref world: WorldStorage,
            game_id: felt252,
            from_x: u8, from_y: u8,
            to_x: u8, to_y: u8,
            ...
            if from_x == to_x {
                ...
                let mut i: u8 = start;
                while i < end {
                    let sq: Square = world.read_model((game_id, from_x, i));
                    assert(!sq.is_occupied, 'Scout: path blocked');
                    i += 1;
                };
            } else {
                ...
                let mut i: u8 = start;
                while i < end {
                    let sq: Square = world.read_model((game_id, i, from_y));
                    assert(!sq.is_occupied, 'Scout: path blocked');
                    i += 1;
                };
            }
